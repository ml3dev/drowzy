...
- **A whitelist that outgrows Chrome sync says so.** `storage.sync` caps one item at 8 KB, and a long whitelist reached it with a raw quota error in the toast. Adding now reports "The whitelist is full for Chrome sync" and leaves the stored list untouched.
...
- In-app What's New page updated to 1.5.0 (`changelog.html`): five short entries written for people who just use the extension (`changelog150Title1..5`, `changelog150Desc1..5`), no internals, the technical detail stays in this file. Earlier releases are collapsed to their titles, each one sliding open on click (native `<details>`, with a short height and fade animation from `changelog.js` that steps aside for reduced-motion users), so the page reads as a timeline and the current release gets the room. It opens once after this update (`SHOW_CHANGELOG_VERSIONS` lists 1.5.0 alongside 1.4.0), the way 1.4.0 did.
